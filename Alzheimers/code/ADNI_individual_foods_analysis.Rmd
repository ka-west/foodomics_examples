...
# merge data, metadata, ontology into one object
ps <- phyloseq(otu_table(f_counts_mat, taxa_are_rows = FALSE),
               tax_table(ont[,c("filename", "ontology_terminal_leaf", "sample_type_group1", "sample_type_group2", "sample_type_group3",
                                "sample_type_group4", "sample_type_group5", "sample_type_group6")]),
               # tax_table(ont[,c("sample_type_group4", "filename")]),
               sample_data(meta))
```
...
```{r}
ggsave(filename = paste0(params$save_dir, "correlations_between_PC1_food_categories.pdf"), plot = p, height = 7, width = 9)
```

## Plot PCA scores colored by each significant ontology category (L3-L5). Saved as interactive html files.
...
# get sig_foods L5
m <- mixOmics::pls(df1 %>%
                     select(one_of(str_replace_all(sig_foods$food, c("mushroom" = "mushroom.y",
                                                                     "cream" = "cream.y",
                                                                     "pig" = "pig.y")))) %>%
                     rename("mushroom" = "mushroom.y",
                            "cream" = "cream.y",
                            "pig" = "pig.y"), 
                   df1 %>%
                     select(RAVLT_immediate, RAVLT_learning, RAVLT_forgetting, RAVLT_perc_forgetting),
                   mode = "canonical")
mixOmics::cim(m, transpose = T, save = "pdf", name.save = paste0(params$save_dir, "PLS_foods_L5_cognition"), 
...
m <- mixOmics::pls(df1 %>%
                   ...
                   df2,
                   mode = "canonical")
mixOmics::cim(m, transpose = T, save = "pdf", name.save = paste0(params$save_dir, "PLS_foods_L5_imp_var"), 
              row.cex = 0.5, col.cex = 0.6)
```

Model aggregated counts

```{r}
# for (fc in c(f_counts_L3, f_counts_L4, f_counts_L5, f_counts_L6)) {
#   f_mat <- fc[meta$class == "ADNI_Go2",]
#   m <- mixOmics::pca(f_mat, 
#                      ncomp = 2, center = T, scale = T)
#   df <- data.frame(m$variates$X, filename = m$names$sample, total_sum = rowSums(f_mat),
#                    unique_foods = apply(fc, 1, function(x) {sum(x > 0)}))
#   df <- left_join(df, meta) %>%
#     left_join(mutate(get(fc), filename = row.names(get(fc))), by = "filename")
#   
#   for (f in c(colnames(f_mat), "total_sum", "unique_foods")) {
#     p <- plot_ly(df, x = ~PC1, y = ~PC2, type = "scatter", mode = "markers", color = ~get(f), symbol = ~class,
#                  marker = list(size = 10, width = 2, line = list(color = "black", width = 2))) %>%
#       layout(title = str_replace_all(f, c("\\.x" = "", "\\.y" = "")),
#              xaxis = list(title = paste0("PC1", " [", round(m$explained_variance[1]*100, digits = 1), "%]"), size = 18),
#              yaxis = list(title = paste0("PC2", " [", round(m$explained_variance[2]*100, digits = 1), "%]"), size = 18))
#     htmlwidgets::saveWidget(p, file = paste0(params$save_dir, "PCA_aggegated_food_counts_colorBY_", fc, "_", f, ".html"))
#   }
# }

```

Model counts (presence/absence only)

```{r}
ps_sub <- subset_samples(ps, class == "ADNI_Go2")
ps_sub <- transform_sample_counts(ps_sub, function(x) {ifelse(x > 0, 1, 0)})
ord <- ordinate(ps_sub, method = "PCoA", distance = "jaccard", binary = TRUE)
ord_plot <- plot_ordination(ps_sub, ordination = ord)
to_plot <- plot_ordination(ps_sub, ordination = ord, justDF = TRUE)
to_plot$total_counts <- sample_sums(ps_sub)
clusters <- kmeans(to_plot[,c("Axis.1", "Axis.2")], centers = 4)
to_plot$Cluster <- clusters$cluster

f_counts_level <- data.frame(f_counts_L6)
to_plot <- left_join(to_plot, mutate(f_counts_level, filename = row.names(f_counts_level)), by = "filename")

food_var <- apply(f_counts_level, 2, var) %>%
  sort(decreasing = TRUE)

p <- plot_ly(to_plot, x = ~Axis.1, y = ~Axis.2, type = "scatter", mode = "markers", color = ~Cluster,
               marker = list(size = 10, width = 2, line = list(color = "black", width = 2))) %>%
    layout(title = "Patient stratification by diet readout",
           xaxis = list(title = ord_plot$labels$x, size = 18),
           yaxis = list(title = ord_plot$labels$y, size = 18))
htmlwidgets::saveWidget(p, file = paste0(params$save_dir, "PCoA_jaccard_food_counts_colorBY_", "cluster", ".html"))

for (f in c(names(food_var[1:30]), "total_counts")) {
  p <- plot_ly(to_plot, x = ~Axis.1, y = ~Axis.2, type = "scatter", mode = "markers", color = ~get(f), #symbol = ~Cluster,
               marker = list(size = 10, width = 2, line = list(color = "black", width = 2))) %>%
    layout(title = str_replace_all(f, c("\\.x" = "", "\\.y" = "")),
           xaxis = list(title = ord_plot$labels$x, size = 18),
           yaxis = list(title = ord_plot$labels$y, size = 18))
  htmlwidgets::saveWidget(p, file = paste0(params$save_dir, "PCoA_jaccard_food_counts_colorBY_", f, ".html"))
}
```

# Focus on important variables from RIMA

ADAS13 score, APOE4, and TAU
